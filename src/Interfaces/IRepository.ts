...
export class IRepository<TQuery> {
  ...
  async getById(keys: TQuery): Promise<typeof this.table.$inferSelect | null> {
    const filter = this.QueryBuilder(keys);
  ...
  async update(
    keys: TQuery,
    Data: Partial<typeof this.table.$inferSelect>
  ): Promise<typeof this.table.$inferSelect | null> {
    const filter = this.QueryBuilder(keys);
