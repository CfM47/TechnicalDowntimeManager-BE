...
import {MaintenanceQuery, MaintenanceQueryBuilder, maintenanceSchema} from "./utils";
import {NewMaintenance, Maintenance} from "../../db/schemas/maintenance";
...
export class MaintenanceController{
  ...
  getById = async(req: Request, res: Response) => {
    try {
      ...
      const maintenanceQuery: MaintenanceQuery = {id_technician: id_technician, id_equipment: id_equipment, date: date};

      const filter = MaintenanceQueryBuilder(maintenanceQuery);
      const maintenanceFound = await this.maintenanceModel.getById(filter);
      if (!maintenanceFound) {
  ...
  update = async (req: Request, res: Response) => {
    try {
      ...
      const updatedMaintenance = await this.maintenanceModel.update(filter, maintenanceData);
  ...
  delete = async (req: Request, res: Response) => {
    try {
      ...
      await this.maintenanceModel.delete(filter);
